...
bool CBourseMgr::Init()
{
	...
	{	
		...
		if (!grdItemBuy) 
			return Error(RES_STRING(CMISS_000446),
...
void CBourseMgr::BuyGoods(CItemCommand& rkBuy, int nFreeCnt)
{
	...
	if (iNum == 0) 
	{
		g_pGameApp->MsgBox(RES_STRING(CMISS_000458));
		return;
	} //end of if 
	...
	{	// �򵥸���Ʒ
		char buf[256] = { 0 };
		sprintf(buf, RES_STRING(CMISS_000460), rkBuy.GetName());
...
bool CBourseMgr::SaleGoods(CItemCommand& rkSaleCmd, int iGridIndex)
{
	...
	{
		char buf[256] = { 0 };
		sprintf(buf, RES_STRING(CMISS_000461), 
...
void CBourseMgr::SetItems()
{
	...
	for (; iIndex < (int)(m_BuyList.size()); ++iIndex)
	{
		...
		{
			LG("Error", RES_STRING(CMISS_000462));
		}
	} //end of for 
						 ...
						 int nGridID)
{
	...
	{
		g_pGameApp->MsgBox(RES_STRING(CMISS_000463));
		return;
	}
	CItemRecord* pkBuyRecord = pkBuyCmd->GetItemInfo();
	if (!pkBuyRecord)	return;
...
bool CBlackTradeMgr::SailToBuy(CGoodsGrid& rkDrag, CGoodsGrid& rkSelf, int nGridID, CCommandObj& rkItem)
{
	...
	CBoxMgr::ShowSelectBox( _TradeExchangeEvent, RES_STRING(CL_LANGUAGE_MATCH_832), true ); // ȷ���Ƿ�һ�

	//if(! pItem->GetIsValid())
	//{
	//	g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_834)); // �һ�ʧ��
	//}
	//else
	//{
	//	CBoxMgr::ShowSelectBox( _TradeExchangeEvent, RES_STRING(CL_LANGUAGE_MATCH_832)); // ȷ���Ƿ�һ�
	//}
...
void CBlackTradeMgr::ExchangeAnswerProc(bool bSuccess, stBlackTrade* pBlackTrade)
{
	if(bSuccess)
	{
		...
		if( nCurNum < g_stUIBlackTrade.grdItemBuy->GetMaxNum())
		{
			...
			{
				char szBuffer[128] = {0};
				sprintf(szBuffer, RES_STRING(CL_LANGUAGE_MATCH_843), pBlackTrade->sSrcNum, pInfo->szName);	// x���ɿ�
				pItem->SetOwnDefText(szBuffer);
			}
		...
		//
		RefreshSaleGrid();
		CCozeForm::GetInstance()->OnSystemMsg(RES_STRING(CL_LANGUAGE_MATCH_833)); // �һ��ɹ�
	}
	else
	{
		CCozeForm::GetInstance()->OnSystemMsg(RES_STRING(CL_LANGUAGE_MATCH_834)); // �һ�ʧ��
	}
}
