...
static LONG WINAPI _VEH_CrashHandler(EXCEPTION_POINTERS* ep) {
	if (ep && ep->ExceptionRecord) {
		DWORD code = ep->ExceptionRecord->ExceptionCode;
		if (code == 0x406D1388) return EXCEPTION_CONTINUE_SEARCH;
		if (code >= 0x80000000) {
			FILE* _f = fopen("log\\veh_crash.log", "a");
			if (_f) {
				fprintf(_f, "[VEH] code=0x%08X addr=%p tid=%lu\n", code, ep->ExceptionRecord->ExceptionAddress, GetCurrentThreadId());
				fflush(_f); fclose(_f);
			}
		}
	}
...
typedef PVOID (WINAPI *AddVEH_t)(ULONG, PVOID);
static void _InstallVEH() {
	auto fn = (AddVEH_t)GetProcAddress(GetModuleHandleA("kernel32.dll"), "AddVectoredExceptionHandler");
	if (fn) fn(1, (PVOID)_VEH_CrashHandler);
}
...
{
	{
		...
		if(hUser32)
		{
			auto fn = (SetProcessDpiAwarenessContext_t)GetProcAddress(hUser32, "SetProcessDpiAwarenessContext");
			if(fn) fn((HANDLE)-1);
		}
	}
	...
	LG("init", "[TRACE0] pre GameLoading\n");
	GameLoading loading;
	if (!g_Config.m_bEditor) {
		loading.Create(strParam);
	}
	LG("init", "[TRACE0] post GameLoading\n");

	::_setmaxstdio(2048);

	setlocale(LC_CTYPE, RES_STRING(CL_LANGUAGE_MATCH_0));
	LG("init", "[TRACE0] post setlocale\n");

	{FILE*_tf=fopen("log\\main_trace.log","a");if(_tf){fprintf(_tf,"[MT] post setlocale\n");fflush(_tf);fclose(_tf);}}

	InstallFont(".\\Font");
	{FILE*_tf=fopen("log\\main_trace.log","a");if(_tf){fprintf(_tf,"[MT] post InstallFont\n");fflush(_tf);fclose(_tf);}}
	LG("init", "[TRACE0] post InstallFont\n");

	dx_ver = 0x900;
	{FILE*_tf=fopen("log\\main_trace.log","a");if(_tf){fprintf(_tf,"[MT] post dx_ver\n");fflush(_tf);fclose(_tf);}}
	LG("init", "[TRACE0] skipped CheckDxVersion (DXVK mode)\n");

	HANDLE	hFile = CreateFileMapping(INVALID_HANDLE_VALUE, 0, PAGE_READWRITE, 0, sizeof(long), "KopClinetNO");
	{FILE*_tf=fopen("log\\main_trace.log","a");if(_tf){fprintf(_tf,"[MT] CreateFileMapping=%p\n",(void*)hFile);fflush(_tf);fclose(_tf);}}
	if (!hFile)
	{
		return 0;
	}
	...
	if (strParam.find("pack") != std::string::npos)
	{
		...
		if (pkfile.SaveToPack(pszpath, pszFile))
			...
			LG("ok", RES_STRING(CMISS_000189), pszPos);
		return 0;
	}
...
{FILE*_tf=fopen("log\\main_trace.log","a");if(_tf){fprintf(_tf,"[MT] pre InitLuaPlatform\n");fflush(_tf);fclose(_tf);}}
#ifdef _LUA_GAME
	InitLuaPlatform();
#endif
	{FILE*_tf=fopen("log\\main_trace.log","a");if(_tf){fprintf(_tf,"[MT] post InitLuaPlatform\n");fflush(_tf);fclose(_tf);}}

	LG("init", "[TRACE] after InitLuaPlatform\n");

	MPTimer t;
	t.Begin();
	{FILE*_tf=fopen("log\\main_trace.log","a");if(_tf){fprintf(_tf,"[MT] pre new CGameApp\n");fflush(_tf);fclose(_tf);}}
	g_pGameApp = new CGameApp();
	{FILE*_tf=fopen("log\\main_trace.log","a");if(_tf){fprintf(_tf,"[MT] post new CGameApp\n");fflush(_tf);fclose(_tf);}}
	LG("init", "[TRACE] after new CGameApp\n");
	g_pGameApp->LG_Config(lg_info);
	{FILE*_tf=fopen("log\\main_trace.log","a");if(_tf){fprintf(_tf,"[MT] post LG_Config\n");fflush(_tf);fclose(_tf);}}
	LG("init", "[TRACE] after LG_Config\n");
	g_lootFilter = new LootFilter();
	{FILE*_tf=fopen("log\\main_trace.log","a");if(_tf){fprintf(_tf,"[MT] post LootFilter\n");fflush(_tf);fclose(_tf);}}
	LG("init", "[TRACE] after LootFilter\n");
	...
	if (strParam.find("clu_bin") != std::string::npos) {
		...
		CLU_LoadScript("scripts/lua/res.clu", 1);
	...
	LG("init", "[TRACE] before LoadCustomProp\n");
	if (!g_stUISystem.m_isLoad)
	{
		g_stUISystem.LoadCustomProp();
	}
	LG("init", "[TRACE] after LoadCustomProp\n");
	...
	{FILE*_tf=fopen("log\\main_trace.log","a");if(_tf){fprintf(_tf,"[MT] pre Init %dx%d depth=%d\n",nWidth,nHeight,nDepth);fflush(_tf);fclose(_tf);}}
	LG("init", "[TRACE] calling g_pGameApp->Init(%dx%d depth=%d)\n", nWidth, nHeight, nDepth);
	if (!g_pGameApp->Init(hInstance, szWindowClass, nWidth, nHeight, nDepth, FALSE /*g_Config.m_bFullScreen*/))
	{
		{FILE*_tf=fopen("log\\main_trace.log","a");if(_tf){fprintf(_tf,"[MT] Init FAILED\n");fflush(_tf);fclose(_tf);}}
		LG("init", RES_STRING(CL_LANGUAGE_MATCH_191));
		g_pGameApp->End();
		return 0;
	}
	{FILE*_tf=fopen("log\\main_trace.log","a");if(_tf){fprintf(_tf,"[MT] post Init OK\n");fflush(_tf);fclose(_tf);}}
	...
	{FILE*_tf=fopen("log\\main_trace.log","a");if(_tf){fprintf(_tf,"[MT] init time=%d ms, entering Run()\n",t.End());fflush(_tf);fclose(_tf);}}
...
{
	MessageBox(NULL, RES_STRING(CL_LANGUAGE_MATCH_375), RES_STRING(CL_LANGUAGE_MATCH_185), 0);
}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		break;
	case WM_CLOSE:
		{FILE*_f=fopen("log\\run_trace.log","a");if(_f){fprintf(_f,"[WM] WM_CLOSE received\n");fflush(_f);fclose(_f);}}
		//{
		//	if( g_Config.m_bEditor )
		//		break;
	...
	case WM_DESTROY:
	{
		{FILE*_f=fopen("log\\run_trace.log","a");if(_f){fprintf(_f,"[WM] WM_DESTROY received\n");fflush(_f);fclose(_f);}}
		CS_Logout();
...
void MakeBinTable()
{
	...
	//	MessageBox( NULL, RES_STRING(CMISS_000193), "Info", 0 );
}
...
void StdoutRedirect()
{
	...
	FILE* hf;
	//AllocConsole();
	hCrt = _open_osfhandle((intptr_t)hOutputWrite, _O_TEXT);
	hf = _fdopen(hCrt, "w");
	*stdout = *hf;
...
int InstallFont(const char* pszPath)
{
	...
	//{
	//	// simsun already in system fonts - skip loading it from custom path
	//}
	//else
	//{
	...
	sprintf(szBuffer, "%s\\Browd.ttf", pszPath);
	if(-1 != access(szBuffer, 0))
		nRet += AddFontResource(szBuffer);

	sprintf(szBuffer, "%s\\COOPBL.TTF", pszPath);
	if(-1 != access(szBuffer, 0))
		nRet += AddFontResource(szBuffer);
	...
	if (nRet)
	{
		...
		SendMessageTimeout(HWND_BROADCAST, WM_FONTCHANGE, 0, 0,
			SMTO_ABORTIFHUNG | SMTO_NOTIMEOUTIFNOTHUNG, 2000, &dwResult);
	}
