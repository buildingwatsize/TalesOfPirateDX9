...
{
	_pNumberBox = g_stUIBox.ShowNumberBox(_evtChallengeNumInput, -1, RES_STRING(CL_LANGUAGE_MATCH_936), true);	// "�����뾹�ļ۸�"
...
void CChurchChallengeMgr::CheckChallengePrice(void)
{
	if(_pNumberBox)
	{
		...
		{
			char szBuf[256] = {0};
			sprintf(szBuf, RES_STRING(CL_LANGUAGE_MATCH_940), _nPrice);	// "ȷ�����ļ۸� %d"
			g_stUIBox.ShowSelectBox(_evtChallengeNumCheck, szBuf, true);
		}
		else
		{
			g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_937), _stInfo.nCurPrice + _stInfo.nMinbid);//"���õ�����ͼ� %d"
		}
	}
}
