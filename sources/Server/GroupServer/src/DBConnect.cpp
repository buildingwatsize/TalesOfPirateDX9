...
static bool TryCreateMasterTable(SQLHDBC hdbc, const char* tableName)
{
	char sql[SQL_MAXLEN];
	sprintf(sql,
		...
		"PRIMARY KEY (cha_id1, cha_id2))",
		tableName, tableName);

	SQLHSTMT hstmt = SQL_NULL_HSTMT;
	SQLRETURN sr = SQLAllocHandle(SQL_HANDLE_STMT, hdbc, &hstmt);
	if (sr != SQL_SUCCESS && sr != SQL_SUCCESS_WITH_INFO) return false;

	sr = SQLExecDirect(hstmt, (SQLCHAR*)sql, SQL_NTS);
	bool ok = (sr == SQL_SUCCESS || sr == SQL_SUCCESS_WITH_INFO);
	SQLFreeHandle(SQL_HANDLE_STMT, hstmt);
	if (ok) {
		printf("[GP] Created missing table '%s'\n", tableName); fflush(stdout);
	}
	return ok;
}
...
bool TBLMaster::InitMasterRelation(map<uLong, uLong> &mapMasterRelation)
{
	...
	for (int attempt = 0; attempt < 2 && !ret; ++attempt)
	{
		...
		catch (...)
		{
			...
			l_line<<newln<<"Exception in InitMasterRelation() attempt "<<(attempt+1);
			...
			if (attempt == 0) {
				...
				TryCreateMasterTable(_hdbc, _get_table());
				continue;
			}
		}
